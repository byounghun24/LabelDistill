...
class LabelDistillModel(BaseLabelDistillModel):
    ...
    #                     cam_mean = feats_cam[instance_mask].mean(dim=0, keepdim=True)    # (1, C)
    #                     lidar_mean = feats_lidar[instance_mask].mean(dim=0, keepdim=True)  # (1, C)

    #                     all_anchors.append(cam_mean)
    #                     all_anchor_labels.append(class_id)
    #                     all_positives.append(lidar_mean)
    #                     all_positive_labels.append(class_id)

    #         if len(all_anchors) < 1 or len(all_positives) < 1:
    #             continue

    #         anchors = torch.cat(all_anchors, dim=0)           # (Na, C)
    #         anchor_labels = torch.tensor(all_anchor_labels, device=anchors.device)  # (Na,)
        ...
        for level, (cam_feats, lidar_feats) in enumerate(zip(cam_feats_list, lidar_feats_list)):
            ...
            for b in range(B):
                ...
                for i, box in enumerate(gt_boxes):
                    ...
                    # ROIAlign 수행
                    rois = torch.tensor([[b, x1, y1, x2, y2]], device=cam_feats.device)
                    cam_roi = roi_align(cam_feats[b:b+1], rois, output_size=(3,3), aligned=True)
                    lidar_roi = roi_align(lidar_feats[b:b+1], rois, output_size=(3,3), aligned=True)
            ...
            ################# Visualize cosine similarity matrix ###################
            classes_vis = ['car', 'truck', 'const_veh', 'bus', 'trailer', 'barrier', 'motor', 'bicycle', 'ped', 'traf_cone']
            cos_sim_np = cos_sim.detach().cpu().numpy()

            anchor_np = anchor_labels.detach().cpu().numpy()  # (Na,)
            positive_np = positive_labels.detach().cpu().numpy()  # (Np,)

            idx_a = np.arange(len(anchor_np))
            idx_p = np.arange(len(positive_np))
            # sort indices by class label
            order_a = np.argsort(anchor_np)
            order_p = np.argsort(positive_np)
            # reorder similarity matrix and IDs
            cos_sim_np = cos_sim.detach().cpu().numpy()
            cos_sorted = cos_sim_np[order_a][:, order_p]
            yticks = [f"{classes_vis[anchor_np[i]]}:{i}" for i in order_a]
            xticks = [f"{classes_vis[positive_np[j]]}:{j}" for j in order_p]
            # plot
            fig, ax = plt.subplots(figsize=(10,8))
            sns.heatmap(cos_sorted, xticklabels=xticks, yticklabels=yticks,
                        cmap="viridis", vmin=-1, vmax=1, ax=ax)
            ax.set_xlabel("LiDAR_feat (class:idx)")
            ax.set_ylabel("Camera_feat (class:idx)")
            ax.set_title("Camera_feat vs LiDAR_feat Cosine Similarity (sorted by class)")
            wandb.log({"cosine_similarity_heatmap": wandb.Image(fig)})
            plt.close(fig)
            ##########################################################################
