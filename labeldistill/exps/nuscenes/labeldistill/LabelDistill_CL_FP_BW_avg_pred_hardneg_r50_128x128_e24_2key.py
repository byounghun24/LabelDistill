...
class LabelDistillModel(BaseLabelDistillModel):
    ...
    def __init__(self, **kwargs):
        ...
        self.topk_negatives = 20
        self.max_pos_per_class = 5
        self.margin = 0.2
    ...
    def compute_contrastive_loss(
        ...
        for lvl, (cam_feats, lidar_feats) in enumerate(zip(cam_feats_list, lidar_feats_list)):
            ...
            A, labA = self.limit_per_class_random(anchors, anchor_labels, self.max_pos_per_class)
            P, labP = self.limit_per_class_random(positives, positive_labels, self.max_pos_per_class)
            ...
            # 4) mask pos/neg
            mask_pos = (labA.unsqueeze(1) == labP.unsqueeze(0))  # (Na, Np)
            mask_neg = ~mask_pos

            # 5) positive score: 클래스당 평균
            pos_sum   = (logits * mask_pos.float()).sum(dim=1)                    # (Na,)
            pos_count = mask_pos.sum(dim=1).clamp(min=1).float()                  # (Na,)
            pos_score = pos_sum / pos_count                                      # (Na,)

            # 6) hard negative mining: anchor별 동적 k
            neg_scores = []
            for i in range(logits.size(0)):                                       # Na만큼 반복
                neg_vals = logits[i][mask_neg[i]]                                 # (num_neg_i,)
                num_neg  = neg_vals.size(0)
                if num_neg > 0:
                    k_i       = min(self.topk_negatives, num_neg)                 # dynamic k
                    topk_vals, _ = torch.topk(neg_vals, k=k_i)                   # (k_i,)
                    neg_scores.append(topk_vals.mean())
                else:
                    # negative가 하나도 없으면 0으로 처리
                    neg_scores.append(torch.tensor(0.0, device=logits.device))
            neg_score = torch.stack(neg_scores)                                  # (Na,)

            # 7) margin-based loss
            losses = F.relu(self.margin + neg_score - pos_score)                 # (Na,)
    ...
    def limit_per_class_random(self, feats, labels, max_per_class: int):
        # feats: list of (1, C) tensors
        # labels: list of int or 0-dim tensor
        per_cls = defaultdict(list)
        for feat, lbl in zip(feats, labels):
            cls = int(lbl)
            per_cls[cls].append(feat)

        out_feats, out_labels = [], []
        for cls, vecs in per_cls.items():
            n_keep = min(len(vecs), max_per_class)
            idxs = torch.randperm(len(vecs), device=vecs[0].device)[:n_keep]
            for i in idxs:
                out_feats.append(vecs[i])
                out_labels.append(cls)

        if out_feats:
            F = torch.cat(out_feats, dim=0)
            L = torch.tensor(out_labels, dtype=torch.long, device=F.device)
        else:
            C = feats[0].shape[1] if feats else 0
            dev = feats[0].device if feats else 'cpu'
            F = torch.empty((0, C), device=dev)
            L = torch.empty((0,), dtype=torch.long, device=dev)
        return F, L
